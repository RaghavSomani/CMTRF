 ...
 */
pair<MatrixXd,MatrixXd>* initialize_UV(int n, int p, int k, int L)
{
	// Mean M and standard deviation set to 1
	double M = 0.5*(L+1), S = 1.0;
...
double MAE(SpMatR& M, MatrixXd& U, MatrixXd& V,set<pair<int,int> >& set_ij)
{
	double temp, loss = 0, total_loss = 0;
	int i,j;
	for(auto ij:set_ij)
	{
		i = ij.first;
...
double MAE_1(SpMatR& M, MatrixXd& U, MatrixXd& V,set<pair<int,int> >& set_ij,tk::spline& sig_inv)
{
	double loss = 0, temp;
	int i,j;
	for(auto ij:set_ij)
...
double MAE_N(SpMatR& M, MatrixXd& U, MatrixXd& V,set<pair<int,int> >& set_ij,tk::spline sig_inv[],set<int> users_with_single_rating)
{
	double loss = 0, temp;
	int i,j;
	for(auto ij:set_ij)
...
double MAE_K(SpMatR& M, MatrixXd& U, MatrixXd& V,set<pair<int,int> >& set_ij,tk::spline sig_inv[], VectorXi& cluster, int K)
{
	double loss = 0, temp;
	int i,j;
	for(auto ij:set_ij)
 ...
 */
double KT(SpMatC& M_testC, SpMatR& M_testR, MatrixXd& U, MatrixXd& V, string file)
{
	int Msz=0, M_predsz=0, p = M_testR.cols(), total = 0;
	VectorXd Msub(p);
	VectorXd M_predsub(p);
	double sum = 0.0, ktuser;
	ofstream kt(file.c_str());
	for(int i=0;i<M_testR.rows();i++)
	{
		...
		ktuser = KendalTau(Msub.head(Msz),M_predsub.head(M_predsz),Msz);
		kt<<i<<" "<<ktuser<<" "<<Msz<<endl;
		sum += ktuser;
		if(Msz>1)
			total++;
	}
...
double DCG(double a[], int len, int level)
{
	double dcg = 0.0;
...
double NDCG(const VectorXd& arr1, const VectorXd& arr2, int len, int level, ofstream& ndcg_file)
{
	...
	for(int i=0;i<len;i++)
	{
		a[i] = V[i].first;
		b[i] = V[i].second;
		ndcg_file<<a[i]<<" "<<b[i]<<endl;
	}
	double dcg = DCG(b,len,level);
	sort(b,b+len,greater<double>());
	double idcg = DCG(b,len,level);
	return dcg/idcg;
}

double ERR(const VectorXd& arr1, const VectorXd& arr2, int len, int level, ofstream& ndcg_file)
{
	vector<pair<double,double> > V;
	for(int i=0;i<len;i++)
		V.push_back(make_pair(arr1(i),arr2(i)));
	...
	double err = 0, p = 1.0, R = 1.0;
	for(int i=0;i<level;i++)
	{
		R = (pow(2,b[i]-1)-1)/pow(2,4);
		err += p*R/(i+1);
		p *= (1-R);
	}
	vector<pair<double,double> > W;
	for(int i=0;i<len;i++)
		W.push_back(make_pair(arr2(i),arr1(i)));
	sort(W.begin(),W.end(),greater<pair<double,double> >());
	for(int i=0;i<len;i++)
		ndcg_file<<W[i].first<<" "<<W[i].second<<endl;
	return err;
}

pair<double,double> NDCG_Matrix(SpMatC& M_testC, SpMatR& M_testR, MatrixXd& U, MatrixXd& V, string file, int threshold, int level)
{
	pair<double,double> z;
	int Msz=0, M_predsz=0, p = M_testR.cols(), total = 0;
	VectorXd Msub(p);
	VectorXd M_predsub(p);
	double sum = 0.0, ndcguser, med = 0;
	ofstream ndcg_file(file.c_str());
	vector<double> ndcg_vec;
	for(int i=0;i<M_testR.rows();i++)
	{
		M_predsz = 0;
		Msz = subvec(M_testC,M_testR,M_testC,M_testR,1,i,&Msub);
		...
		if(Msz>threshold)
		{
			ndcguser = ERR(M_predsub.head(M_predsz),Msub.head(Msz),Msz,level,ndcg_file);
			// for(int j=0;j<Msz;j++)
			// 	ndcg_file<<M_predsub(j)<<" "<<Msub(j)<<endl;
			ndcg_file<<i<<" "<<ndcguser<<" "<<Msz<<endl;
			sum += ndcguser;
			ndcg_vec.push_back(ndcguser);
			total++;
		}
	}
	ndcg_file.close();
	sort(ndcg_vec.begin(),ndcg_vec.end());
	if(ndcg_vec.size()%2==0)
		med = ndcg_vec[(ndcg_vec.size()-1)/2];
	else
		med = 0.5*( ndcg_vec[ndcg_vec.size()/2] + ndcg_vec[(ndcg_vec.size()/2)-1] );
	z.first = sum/total;
	z.second = med;
	double var = 0;
	for(auto n:ndcg_vec)
		var += (n-z.first)*(n-z.first);
	cout<<"Std dev = "<<sqrt(var/total)<<endl;
	return z;
}

pair<double,double> NDCG_Matrix_1sigma(SpMatC& M_testC, SpMatR& M_testR, MatrixXd& U, MatrixXd& V, string file, tk::spline& sig_inv, int threshold, int level)
{
	pair<double,double> z;
	int Msz=0, M_predsz=0, p = M_testR.cols(), total = 0;
	VectorXd Msub(p);
	...
	for(int i=0;i<M_testR.rows();i++)
	{
		...
		Msz = subvec(M_testC,M_testR,M_testC,M_testR,1,i,&Msub);
		for (SpMatR::InnerIterator it(M_testR,i); it; ++it)
			M_predsub(M_predsz++) = sig_inv(U.row(i).dot(V.row(it.col())));
		if(Msz>threshold)
		{
			ndcguser = ERR(M_predsub.head(M_predsz),Msub.head(Msz),Msz, level, ndcg_file);
			// for(int j=0;j<Msz;j++)
...
pair<double,double> NDCG_Matrix_Nsigma(SpMatC& M_testC, SpMatR& M_testR, MatrixXd& U, MatrixXd& V, string file,tk::spline sig_inv[],set<int> users_with_single_rating, int threshold, int level)
{
	pair<double,double> z;
	int Msz=0, M_predsz=0, p = M_testR.cols(), total = 0;
	VectorXd Msub(p);
	...
	for(int i=0;i<M_testR.rows();i++)
	{
		...
		Msz = subvec(M_testC,M_testR,M_testC,M_testR,1,i,&Msub);
		for (SpMatR::InnerIterator it(M_testR,i); it; ++it)
			M_predsub(M_predsz++) = sig_inv[i](U.row(i).dot(V.row(it.col())));
		if(Msz>threshold)
		{	
			ndcguser = ERR(M_predsub.head(M_predsz),Msub.head(Msz),Msz,level,ndcg_file);
			// for(int j=0;j<Msz;j++)
...
pair<double,double> NDCG_Matrix_Ksigma(SpMatC& M_testC, SpMatR& M_testR, MatrixXd& U, MatrixXd& V, string file, tk::spline sig_inv[], VectorXi& cluster, int K, int threshold, int level)
{
	pair<double,double> z;
	int Msz=0, M_predsz=0, p = M_testR.cols(), total = 0;
	VectorXd Msub(p);
	...
	for(int i=0;i<M_testR.rows();i++)
	{
		...
		Msz = subvec(M_testC,M_testR,M_testC,M_testR,1,i,&Msub);
		for (SpMatR::InnerIterator it(M_testR,i); it; ++it)
			M_predsub(M_predsz++) = sig_inv[cluster(i)](U.row(i).dot(V.row(it.col())));
		if(Msz>threshold)
		{	
			ndcguser = ERR(M_predsub.head(M_predsz),Msub.head(Msz),Msz,level,ndcg_file);
			// for(int j=0;j<Msz;j++)
 ...
 * target_y = double array of size 5 storing the averages for each rating {1,2,3,4,5}
 */
int get_Vui_mean(VectorXd& M_expanded, const Ref<VectorXd>& Vui_expanded, double target_y[],int L)
{	
	Array<bool,Dynamic,1> mask;
	...
	for(int rating=1;rating<=L;rating++)
	{
		...
		if(mask.count())
		{
			for(int i=0,j=0;i<mask.size() && i<Vui_expanded.size();i++)	
				if(mask(i))
 ...
 */
double* get_interp_Sigma_i(int target_y_count, double target_y[], double margins[], double W[], int L)
{
	// target_y_count = 0;
	// for(int i=0;i<5;i++)
	// 	if(target_y[i]!=0)
	...
	if(target_y_count==L)
		z = margin_isotonic(target_y,W,margins,L);
	//if target_y has 2, 3 or 4 number of ratings averages, compute spline using only those
	else if(target_y_count<L && target_y_count>1)
	{
		for(int rating=1;rating<=L;rating++)
			if(target_y[rating-1]!=0.0)
			{
 ...
 */
double* get_1_sigma(SpMatC& M_trainC, SpMatR& M_trainR, VectorXd& M_expanded, MatrixXd& U, MatrixXd& V, double margins[], double W[], VectorXi& cluster, int k, int L)
{
	double target_y[L];
	...
	int target_y_count = get_Vui_mean(M_expanded,Vui_expanded.head(Vui_expanded_size),target_y,L);
	return get_interp_Sigma_i(target_y_count,target_y,margins,W,L);
}
 ...
 */
void getSigmaInv(double z[], tk::spline& sig_inv,int L)
{
 ...
 */
double* getSigma_i(SpMatC& M_trainC, SpMatR& M_trainR, MatrixXd& U, MatrixXd& V, double margins[], double W[], int i, int L)
{
	double* z;
	double target_x[L];// = {1,2,3,4,5};
	for(int j=0;j++;j<L)
		target_x[j] = j+1;
	double target_y[L];// = {0,0,0,0,0};
	fill_n(target_y,L,0);
	double sum = 0.0, temp_vec_mean;
	...
	for(int rating=1;rating<=L;rating++)
	{
		...
		if(mask.count())
		{
			temp_vec_mean = temp_vec.mean();
			if(temp_vec_mean!=0)
			{
				target_y[rating-1] = temp_vec_mean;
				target_y_count++;
			}
		}
	}
	...
	z = get_interp_Sigma_i(target_y_count,target_y,margins,W,L);
	return z;
}
 ...
 */
double row_loss(SpMatR& M_trainR, MatrixXd& U, MatrixXd& V, double* sigma[], int i, int k, int& count)
{
...
int assign_sigma(SpMatR& M_trainR, MatrixXd& U, MatrixXd& V, double* z_cluster[], VectorXi& cluster, int K)
{
	...
	for(int i=0;i<M_trainR.rows();i++)
	{
		...
		for(int k=0;k<K;k++)
		{
			temp = row_loss(M_trainR,U,V,z_cluster,i,k,count);
 ...
 * 				each pointing to double array of size 5 storing the pooled weights
 * 				of each cluster
 ...
 */
void update_W(SpMatR& M_trainR, SpMatR& M_testR, double* W_cluster[], int N_total_train[], int N_total_test[], VectorXi& cluster, int k, set<int> users_with_single_rating, int L)
{
	int N[k][L];
	memset(N, 0, sizeof(N[0][0])*k*L);
	// int N_total[k];
	...
	for(int i=0;i<k;i++)
		transform(N[i], N[i]+L, W_cluster[i],bind2nd(divides<double>(),(double)N_total_train[i]));
}

/* Computes the testloss from the i^th row of a matrix
 * Row loss for i^th row = sum{j such that M(i,j)!=0} { ( M(i,j) - sig^-1(<U_i,V_j>) )^2 }
 * Takes inputs
 * M_testR (n x p) = Sparse Matrix for computing row testloss, in Row Major form
 * U (n x k), V (p x k) = factored matrices
 * sig_inv_cluster = array of splines (assuming precomputed) of sigma inverse functions each cluster (n x 5)
 * cluster = Vector of length n storing the cluster number a user row belongs to, in {-1,0,1,...,K-1}
 * 				-1 represents that the user has been alotted no cluster
 * i = the row index of M_train for which row loss has to be calculated
 */
double row_testloss(SpMatR& M_testR, MatrixXd& U, MatrixXd& V, tk::spline sig_inv_cluster[], VectorXi& cluster, int i, int k, int& count)
{
	double temp = 0, loss = 0;
	count = 0;
	if(k<0)
	...
	for(SpMatR::InnerIterator it(M_testR,i); it; ++it)
	{
		temp = sig_inv_cluster[cluster(i)](U.row(i).dot(V.row(it.col()))) - M_testR.coeffRef(i,it.col());
		loss += temp*temp;
		count++;
	}
	return loss;
...
void user_loss(SpMatR& M_testR, MatrixXd& U, MatrixXd& V, tk::spline sig_inv_cluster[], VectorXi& cluster)
{	
	ofstream lossfile("rowtestloss.txt");
	double rl;
	int count = 0;
	for(int i=0;i<M_testR.rows();i++)
	{
		rl = row_testloss(M_testR,U,V,sig_inv_cluster,cluster,i,cluster(i),count);
		lossfile<<i<<" "<<(count>0?(rl/count):0)<<" "<<count<<endl;
	}
	lossfile.close();
}