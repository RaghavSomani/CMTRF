...
int main()
{
	...
	int dataset;
	cin>>dataset;
	int i,j,r,n,p,split,k=0,ndcg_thresh = 30, L = 5;
	...
	double W[L], margins[L-1], target_y[L];
	fill_n(margins, L-1, eps);
	transform(N, N+L, W,bind2nd(divides<double>(),(double)N_total));
	...
	for(auto l1:L1)
	{
		for(auto l2:L2)
		{
			...
			for(int t=0;1;t++)
			{
				...
				z = get_1_sigma(M_trainC,M_trainR,M_expanded,U,V,margins,W,cluster,0,L);

				// computing the respective sigma inverse in a linear spline
				
				getSigmaInv(z,sig_inv,L);
